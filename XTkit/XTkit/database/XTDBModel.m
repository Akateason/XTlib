...
+ (BOOL)createTable
{
    ...
    if (![[XTFMDB sharedInstance] verify]) return FALSE ;
    
    __block BOOL bReturn = FALSE ;
    
    if(![[XTFMDB sharedInstance] isTableExist:tableName])
    {
...
{
    NSString *tableName = NSStringFromClass([self class]) ;
    if (![[XTFMDB sharedInstance] verify]) return -1 ;
    if(![[XTFMDB sharedInstance] isTableExist:tableName]) return -2 ;
...
{
    if (![[XTFMDB sharedInstance] verify]) return FALSE ;
    if (![[XTFMDB sharedInstance] isTableExist:NSStringFromClass([[modelList firstObject] class])]) return FALSE ;
...
- (BOOL)update
{
    NSString *tableName = NSStringFromClass([self class]) ;
    if (![[XTFMDB sharedInstance] verify]) return FALSE ;
    if (![[XTFMDB sharedInstance] isTableExist:tableName]) return FALSE ;
...
{
    NSString *tableName = NSStringFromClass([self class]) ;
    if (![[XTFMDB sharedInstance] verify]) return nil ;
    if(![[XTFMDB sharedInstance] isTableExist:tableName]) return nil ;
